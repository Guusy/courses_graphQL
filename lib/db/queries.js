...
const { COURSE, STUDENT } = require('./ENTITIES')
...
const globalSearch = async ({ keyword }) => {
  ...
  try {
    const db = await connectDb()
    const courses = await searchByText({ text: keyword, db, collection: COURSE.collection })
    const students = await searchByText({ text: keyword, db, collection: STUDENT.collection })
...
const searchByText = ({ db, collection, text }) => {
